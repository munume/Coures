...
def run_numpy_pb(numpy_pb, image):
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            input_tensor = sess.graph.get_tensor_by_name("X_content:0")
            out_tmp_tensor_1 = sess.graph.get_tensor_by_name("Conv2D_13:0") #input from node B
            out_tmp_tensor_2 = sess.graph.get_tensor_by_name("moments_15/StopGradient:0") #input from node B
            ...
            #Node C
            input_tensor_new = sess.graph.get_tensor_by_name("moments_15/PowerDifference:0") #new add node
            output_tensor = sess.graph.get_tensor_by_name("add_37:0")
