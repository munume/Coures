...
def run_numpy_pb():
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            input_tensor = sess.graph.get_tensor_by_name("X_content:0")
            output_tmp_tensor1 = sess.graph.get_tensor_by_name("Conv2D_13:0")
            output_tmp_tensor2 = sess.graph.get_tensor_by_name("moments_15/StopGradient:0")
            
            start_time = time.time()
            #Node Numpy
            input_x, input_y = sess.run([output_tmp_tensor1, output_tmp_tensor2], feed_dict={input_tensor:[X]})
            input_pow = 2
